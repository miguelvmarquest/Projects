<?php 
    ...
    if (after('proj2/',$_SERVER['REQUEST_URI'])=='searchutilizadores.php'){
        $tipo='name';
    }else{
        $tipo = $_GET["tipo"];
    }
...
if (count($_SESSION) == 0){
	...
	$signin = "<form class='navbar-form navbar-right' action='index.php' method='post'><div class='form-group'>Ola ". $_SESSION["username"] . "</div><div style='margin-left:10px' class='form-group' id='logOut'><input class='btn btn-default' type='submit' name='logout' id='logout' value='Log Out'></div></form>";
...
$(document).ready(function(){
	...
	path = path.split('/');
	var ger = path[5];
	if (ger === "searchutilizadores.php?tipo=date"){
    ...
    $("#datissima").click(function(){
    	window.location.href='searchutilizadores.php?tipo=date';
    });
});
</script>   
...
<body>
    ...
    <div id="banner" class="page-header">
                              ...
                              <!-- Nav tabs -->
                                    <div class="well">
                                          <ul  class="nav nav-pills nav-stacked" role="tablist">
                                            <li onclick="window.location.href='searchutilizadores.php?tipo=name'" style='display:inline-block' role="presentation" class="<?php if ($tipo=="name"){echo 'active';}?> col-md-1.5"  ><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Name</a></li>

                                            <li  onclick="window.location.href='searchutilizadores.php?tipo=description'" style='display:inline-block' class="<?php if ($tipo=="description"){echo 'active';}?> col-md-1.5" role="presentation"><a href="#profile" aria-controls="profile" role="tab" data-toggle="tab">Description</a></li>

                                            <li  onclick="window.location.href='searchutilizadores.php?tipo=keywords'" style='display:inline-block'  class="<?php if ($tipo=="keywords"){echo 'active';}?> col-md-1.5" role="presentation"><a href="#messages" aria-controls="messages" role="tab" data-toggle="tab">Keywords</a></li>
		...
		<?php
			...
			if (!isset($s) || trim($s) == ''){
				    $query = "select id_product, name, description, valor, start_date, end_date, keywords,fotos from products WHERE id_user=$id_user limit 5";
    				$result = mysqli_query($conn, $query) or die("Error ". $query);
                ...
                $sql = "SELECT id_product,$tipo FROM products WHERE id_user=$id_user";
                ...
                if (!count($productsarray)==0){
                    $sql = "SELECT id_product, name, description, valor, start_date, end_date, keywords,fotos FROM products WHERE id_product='$productsarray[0]' AND id_user=$id_user";
