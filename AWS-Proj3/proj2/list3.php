...
function interval(){ 
    setInterval(function(){ showUser(<?php echo $_GET['user'];?>);}, 10000);
}
...
<?php
        ...
        global $conn;
            ...
            $sql="select distinct p.name,p.end_date,p.valor,u.nick,p.id_product from licitations lic, products p, utilizadores u where lic.id_user='$user' and p.id_product=lic.id_product and u.user_id=(select u.user_id from utilizadores u, licitations l where l.id_user=u.user_id and l.valor=(select max(valor) from licitations where id_product=p.id_product)) order by p.end_date DESC";
            $result = mysqli_query($conn,$sql);
            $num_rows = mysqli_num_rows($result);
            $array=array();
            while($row = mysqli_fetch_array($result)) {
                if (strtotime($row[1])>=strtotime(date("Y-m-d"))){
                    array_push($array,"<a href='item2.php?item_id=". $row[4] . "'><div class='row margin-0'>
                                <div class='col-md-3'>
                                    <div class='cell'>
...
<div class="container">
    ...
    <div class="method">
        <div class="row margin-0 list-header hidden-sm hidden-xs">
            ...
            <div class="col-md-2"><div class="header">Best Value <span class='glyphicon glyphicon-bitcoin'></span></div></div>
